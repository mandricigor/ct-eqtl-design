...
def dichotomy(cells, money, multi, eps=0.00001, illumina_per_million=5):
    mini = 1
    maxi = 1000000
    for i in range(20):
...
def exp_design(budget, lo_cell, hi_cell, lo_p, hi_p, diff_cell=250, multi=8, illumina_per_million=5):
    ...
    while pers >= lo_p:
        ...
        while cn >= lo_cell:
            ...
            rpp = dichotomy(cells_batch, seq_batch_budget, multi, illumina_per_million=illumina_per_million)
            if rpp > 0:
...
def exp_design_fixed_lane_capacity(budget, lo_cell, hi_cell, lo_p, hi_p, diff_cell=250, diff_person=8, capacity=24000, max_multi=16, illumina_per_million=5):
    ...
    while pers >= lo_p:
        ...
        while cn >= lo_cell:
            ...
            for v, w in zip(batch_money_info, batch_ind_info):
                u = cn * w
                rpp = dichotomy(u, v, w, illumina_per_million=illumina_per_million)
                ##print rpp, "RPP"
                if rpp > 0:
